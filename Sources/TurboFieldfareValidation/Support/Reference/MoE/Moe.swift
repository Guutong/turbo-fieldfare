...
public enum MoeRef {
    ...
    public static func runFFN(
        ...
        let gateOut = DequantInt4GemvRef.apply(weightRows: gateRows, x: x, n: d, groupSize: groupSize)
        let upOut   = DequantInt4GemvRef.apply(weightRows: upRows,   x: x, n: d, groupSize: groupSize)
        let gated   = geluTanh(gateOut)
        var act = [Float](repeating: 0, count: f)
        vDSP_vmul(gated, 1, upOut, 1, &act, 1, vDSP_Length(f))
        return DequantInt4GemvRef.apply(weightRows: downRows, x: act, n: f, groupSize: groupSize)
    }
