...
public struct ServerArguments: Equatable, Sendable {
    ...
    usage: TurboFieldfareServer --model <completed .gturbo directory> [options]
      ...
      --max-context <tokens> 4096, 8192, or 16384 (default 16384).
        ...
        while index < input.count {
            ...
            case "--max-context":
                guard let parsed = Int(value),
                      ContextCap.allowedServerValues.contains(parsed) else {
                    let allowed = ContextCap.allowedServerValues
                        .map(String.init).joined(separator: ", ")
                    throw ServerArgumentError.invalid(
                        "--max-context must be one of \(allowed) "
                            + "(maximum supported context is \(ContextCap.maximum) tokens); got \(value)")
                }
                maxContext = parsed
            case "--queue-limit":
