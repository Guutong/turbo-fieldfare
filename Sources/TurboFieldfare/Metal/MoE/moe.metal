...
// Fixed-topK=8 fast path (Qwen3.6's topK), unrolling the reduce loop instead
// of the runtime `K` bound above. Reserved by the Swift side
// (MoE.phase2ReduceK8PSO / phase2ReduceK8SpecializedPSO) for a follow-up that
// dispatches it in place of moe_phase2_down_reduce when topK == 8; kept here
// so that reservation resolves to a real kernel rather than a missing-symbol
