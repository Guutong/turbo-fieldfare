...
final class PrefillLayerTail {
    ...
    init(context: MetalContext) throws {
        ...
        self.preNormPSO = try context.pipeline("prefill_layer_tail_prenorm_block")
    }

    /// Pre-norm block variant: `hidden += h1 + routedScale * h2`, with no
    /// output norms and no per-layer residual gain.
    func encodePreNorm(commandBuffer: MTLCommandBuffer,
        ...
        enc.setBytes(&h2Stride, length: MemoryLayout<UInt32>.size, index: 5)
        enc.setBytes(&h1Stride, length: MemoryLayout<UInt32>.size, index: 6)
        enc.setBytes(&hiddenStride, length: MemoryLayout<UInt32>.size, index: 7)
