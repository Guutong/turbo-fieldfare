...
final class PrefillPostAttentionSetup {
    ...
    init(context: MetalContext) throws {
        ...
        self.preNormPSO = try context.pipeline("prefill_post_attn_setup_prenorm_block")
    }

    /// Pre-norm block variant. The attention output joins the residual
    /// unnormed, and `preFFNWeight` (the model's `post_attention_layernorm`)
    /// norms the result into all three FFN/router inputs.
    func encodePreNorm(commandBuffer: MTLCommandBuffer,
                       hidden: MTLBuffer,
                       hiddenOffset: Int = 0,
                       attn: MTLBuffer,
                       attnOffset: Int = 0,
        ...
        enc.setBytes(&hiddenStride, length: MemoryLayout<UInt32>.size, index: 8)
        enc.setBytes(&attnStride, length: MemoryLayout<UInt32>.size, index: 9)
        enc.setBytes(&denseStride, length: MemoryLayout<UInt32>.size, index: 10)
        enc.setBytes(&routedStride, length: MemoryLayout<UInt32>.size, index: 11)
        enc.setBytes(&routerStride, length: MemoryLayout<UInt32>.size, index: 12)
        enc.setBytes(&epsVar, length: MemoryLayout<Float>.size, index: 13)

        let threads = min(preNormPSO.maxTotalThreadsPerThreadgroup, 256)
