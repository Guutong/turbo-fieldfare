...
/// Swift wrapper for `dequant_int5_gemv_simd`.
///
///   y = W * x
///
/// where W (M rows, N cols) is MLX-affine 5-bit (sub-byte packed, see
...
/// this kernel targets pairs 5-bit layer-0 projections (group_size=64) with
...
final class DequantInt5GEMV {
    private static let rowsPerThreadgroup = 8

    private let pso: MTLComputePipelineState

    init(context: MetalContext) throws {
        self.pso = try context.pipeline("dequant_int5_gemv_simd")
    }
